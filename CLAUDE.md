...
1. **🏠 ダッシュボード** — 朝開くと S/A/B/C ランク別件数・優先案件 top5・指値検討中を即把握（デフォルトページ）
2. **💬 AI チャット入力** — PDF/メール本文を貼ると Gemini が会話で30項目を構造化（30項目手入力 → 平均3往復のチャット）
3. **📋 案件分析** — 物件入力フォーム → AI分析 → KPI/スコア/レポート表示／🧠 AIセカンドオピニオン Q&A タブ／🔁 類似過去案件レコメンド
4. **📦 バルク案件** — テキスト一括貼り付け → 複数物件同時スクリーニング
5. **📊 比較分析** — 保存済み案件を横並び比較
6. **📁 保存済み案件** — 履歴管理・再表示
7. **❓ 使い方** — マルチOS別の起動・利用ガイド
